...
def get_points_dict(workbook_path: Path) -> dict:
    ...
    for _, row in table_10.iterrows():
        if " - " in str(row["№№"]):
            ...
            origin = (
                table_79.loc[table_79["№№"] == origin_label]["Y"].values[0],
                table_79.loc[table_79["№№"] == origin_label]["X"].values[0],
            )

            _dest = (
                table_79.loc[table_79["№№"] == dest_label]["Y"].values[0],
                table_79.loc[table_79["№№"] == dest_label]["X"].values[0],
            )

            angle = np.rad2deg(
                get_angle_between_points(origin[0], origin[1], _dest[0], _dest[1])
            )
            _i_am_shit_at_coding = list(origin)
            _i_am_shit_at_coding.append(
                table_79.loc[table_79["№№"] == origin_label]["H"].values[0]
            )
        ...
        point_dict.update(
            {str(row["№№"]): [xx, yy, row["Отметки реечн. точек Hр.т., м"]]}
        )
...
def add_point_marker(
    point_name: str,
    point_cords: tuple[float, float],
    point_height: float,
    radius: float = 1000,
    offset: tuple[float, float] = (15000, 7000),
):
...
def main() -> None:
    for _variant in range(27, 28):
        ...
        for point in (main_points_names := ["ПЗ41", "111", "112", "113", "ПЗ42"]):
            add_point_marker(
                point, m_to_mm(points_dict[point][:2]), points_dict[point][2] * 1000
            )
        ...
        for point in con_111:
            ...
            _alpha = np.rad2deg(
                get_angle_between_points(
                    center_point[0], center_point[1], point[0], point[1]
                )
            )
            ...
            for i in height_dif:
                ...
                _interpolated_point = endpoint_by_distance_and_angle(
                    m_to_mm(tuple(center_point)), _d * 1000, _alpha
                )
        ...
        _max_num_of_points = max(
            [
                len(interpolated_points[height_points])
                for height_points in interpolated_points
            ]
        )
        ...
        for height in interpolated_points:
            ...
            cent = (
                sum([p[0] for p in interpolated_points[height]])
                / len(interpolated_points[height]),
                sum([p[1] for p in interpolated_points[height]])
                / len(interpolated_points[height]),
            )
            interpolated_points[height].sort(
                key=lambda p: atan2(p[1] - cent[1], p[0] - cent[0])
            )

            iDocument2D.ksBezier(
                len(interpolated_points[height]) == _max_num_of_points, 1
            )
        ...
        interpolated_points_fruit_garden[_last_point_between_21_23].insert(
            0, *_interpolation_p42_18[_last_point_between_21_23]
        )
        ...
        iDocument2D.ksBezier(False, 1)
        for point in interpolated_points_behind_fruit_garden[
            _last_point_between_112_10
        ]:
            iDocument2D.ksPoint(*point, 0)
        iDocument2D.ksEndObj()
        ...
        for i in range(len(_left_border) - 1):
            ...
            iDocument2D.ksLineSeg(
                *[
                    i * 1000
                    for i in (
                        *points_dict[_left_border[i]][:2],
                        *points_dict[_left_border[i + 1]][:2],
                    )
                ],
                4,
            )
        ...
        for i in range(len(_top_border) - 1):
            ...
            iDocument2D.ksLineSeg(
                *[
                    i * 1000
                    for i in (
                        *points_dict[_top_border[i]][:2],
                        *points_dict[_top_border[i + 1]][:2],
                    )
                ],
                1,
            )
        ...
        for i in range(len(_p_border) - 1):
            ...
            iDocument2D.ksLineSeg(
                *[
                    i * 1000
                    for i in (
                        *points_dict[_p_border[i]][:2],
                        *points_dict[_p_border[i + 1]][:2],
                    )
                ],
                4,
            )
        ...
        for i in range(len(_river_border) - 1):
            ...
            iDocument2D.ksLineSeg(
                *[
                    i * 1000
                    for i in (
                        *points_dict[_river_border[i]][:2],
                        *points_dict[_river_border[i + 1]][:2],
                    )
                ],
                1,
            )
        ...
        _alpha = np.rad2deg(
            get_angle_between_points(*points_dict["4"][:2], *points_dict["3"][:2])
        )

        _second_shore = []
        _d = 32
        for i in range(len(_river_border)):
            _second_shore.append(
                endpoint_by_distance_and_angle(
        ...
        for i in range(len(_second_shore) - 1):
            if i != len(_second_shore) - 2:
                ...
                iDocument2D.ksLineSeg(
                    *_second_shore[i], *(p * 1000 for p in points_dict["19"][:2]), 1
                )

        # Добавить подпись

        _xx = (points_dict["2"][0] + points_dict["21"][0]) / 2 * 1000 + (
            _d / 2 * 1000 * cos(np.radians(_alpha))
        )
        _yy = (points_dict["2"][1] + points_dict["21"][1]) / 2 * 1000 + (
            _d / 2 * 1000 * sin(np.radians(_alpha))
        )
        ...
        add_text(
            ...
            _yy,
            np.rad2deg(
                get_angle_between_points(*points_dict["2"][:2], *points_dict["21"][:2])
            ),
            5,
        )
        ...
        _alpha = np.rad2deg(
            get_angle_between_points(*points_dict["6"][:2], *points_dict["7"][:2])
        )

        _autobahn = []
        _d = 10
        for i in range(len(_top_border)):
            xx = points_dict[_top_border[i]][0] * 1000 + (
                _d * 1000 * cos(np.radians(_alpha))
            )
            yy = points_dict[_top_border[i]][1] * 1000 + (
                _d * 1000 * sin(np.radians(_alpha))
            )
        ...
        _alpha_14_15 = np.rad2deg(
            get_angle_between_points(*points_dict["14"][:2], *points_dict["15"][:2])
        )
        _alpha_113_P42 = np.rad2deg(
            get_angle_between_points(*points_dict["113"][:2], *points_dict["ПЗ42"][:2])
        )

        # _well_angles = degrees(atan2(14.62,9.15))
        #
        _well_point_1 = m_to_mm(
            endpoint_by_distance_and_angle(points_dict["113"][:2], 40, _alpha_113_P42)
        )
        # _obj_well_point_1 = iDocument2D.ksPoint(*_well_point_1, 0)

        _well_point_2 = m_to_mm(
            endpoint_by_distance_and_angle(
                points_dict["113"][:2], 58.61, _alpha_113_P42
            )
        )
        ...
        _well_point_3 = get_intersections(
            *_well_point_1, 9.15 * 1000, *_well_point_2, 14.62 * 1000
        )
        ...
        fruit_garden_points = [
            ...
            endpoint_by_distance_and_angle(
                endpoint_by_distance_and_angle(
                    points_dict["113"][:2], 133.41, _alpha_113_P42
                ),
                -7.81,
                _alpha_14_15,
            ),
            tuple(points_dict["23"][:2]),
        ]

        iDocument2D.ksLineSeg(
            *m_to_mm(fruit_garden_points[0]), *m_to_mm(fruit_garden_points[3]), 1
        )
        for i in range(len(fruit_garden_points) - 1):
            iDocument2D.ksLineSeg(
                *m_to_mm(fruit_garden_points[i]),
                *m_to_mm(fruit_garden_points[i + 1]),
                1,
            )
        ...
        _alpha_fs = np.rad2deg(
            get_angle_between_points(*fruit_garden_points[1], *fruit_garden_points[2])
        )
        ...
        add_text(
            ...
            *m_to_mm(
                endpoint_by_distance_and_angle(
                    fruit_garden_points[0], 15, _alpha_fs + 30
                )
            ),
            np.rad2deg(
                get_angle_between_points(
                    *fruit_garden_points[0], *fruit_garden_points[1]
                )
            ),
            5,
        )
        ...
        _kg2_p1 = m_to_mm(
            endpoint_by_distance_and_angle(
                endpoint_by_distance_and_angle(
                    points_dict["113"][:2], 80.05, _alpha_113_P42
                ),
                -8.03,
                _alpha_14_15,
            )
        ...
        _kg2_p2 = m_to_mm(
            endpoint_by_distance_and_angle(
                endpoint_by_distance_and_angle(
                    points_dict["113"][:2], 110.23, _alpha_113_P42
                ),
                -7.91,
                _alpha_14_15,
            )
        ...
        add_text(
            ...
            _kg2_p2[0] - 2400,
            _kg2_p2[1] + 4000,
            np.rad2deg(get_angle_between_points(*_kg2_p2, *_kg2_p1)),
            5,
        )
        ...
        _railroad_p1 = endpoint_by_distance_and_angle(
            points_dict["113"][:2], 12.64, _alpha_14_15
        )
        _railroad_end = endpoint_by_distance_and_angle(
            endpoint_by_distance_and_angle(
                points_dict["113"][:2], 162.1, _alpha_113_P42
            ),
            14.28,
            _alpha_14_15,
        )

        _alpha_rail = np.rad2deg(
            get_angle_between_points(*_railroad_p1, *_railroad_end)
        )
        ...
        for i in range(5, int(line_len(_railroad_p0, _railroad_end)) + 5, 5):
            ...
            _side_point_1 = endpoint_by_distance_and_angle(
                _start_point, 2.5, 45 - _alpha_rail
            )
            _side_point_2 = endpoint_by_distance_and_angle(
                _start_point, -2.5, 45 - _alpha_rail
            )
        ...
        _plan_poly = [
            m_to_mm(tuple(points_dict["20"][:2])),
            m_to_mm(tuple(points_dict["21"][:2])),
            m_to_mm(tuple(points_dict["2"][:2])),
            m_to_mm(tuple(points_dict["4"][:2])),
            m_to_mm(tuple(points_dict["5"][:2])),
            m_to_mm(tuple(points_dict["6"][:2])),
            m_to_mm(tuple(points_dict["7"][:2])),
            m_to_mm(tuple(points_dict["8"][:2])),
            m_to_mm(tuple(points_dict["10"][:2])),
            m_to_mm(tuple(points_dict["14"][:2])),
            m_to_mm(tuple(points_dict["15"][:2])),
        ...
        _farm_poly = [
            m_to_mm(tuple(points_dict["ПЗ41"][:2])),
            m_to_mm(tuple(points_dict["5"][:2])),
            m_to_mm(tuple(points_dict["6"][:2])),
            m_to_mm(tuple(points_dict["7"][:2])),
            m_to_mm(tuple(points_dict["8"][:2])),
        ...
        _fruit_garden_poly = geometry.Polygon(
            geometry.LineString((m_to_mm(i) for i in fruit_garden_points))
        )
        ...
        for i in range(
            int(points_dict["20"][1] - int(points_dict["20"][1]) % 5),
            int(points_dict["7"][1]),
            20,
        ):
            for b in range(
                int(points_dict["4"][0] - int(points_dict["4"][1]) % 5),
                int(points_dict["15"][0]),
                20,
            ):
                _d_point = geometry.Point(m_to_mm((b, i)))
                if (
                    _plan_poly.contains(_d_point)
                    and not _farm_poly.contains(_d_point)
                    and not _fruit_garden_poly.contains(_d_point)
                ):
        ...
        for i in range(
            int(fruit_garden_points[2][1]), int(fruit_garden_points[0][1]), 5
        ):
            for b in range(
                int(fruit_garden_points[-1][0]), int(fruit_garden_points[1][0]), 5
            ):
                _d_point = geometry.Point(m_to_mm((b, i)))
                if _fruit_garden_poly.contains(_d_point) and not _kg_poly.contains(
                    _d_point
                ):
        ...
        for i in range(
            int(points_dict["4"][1] - int(points_dict["4"][1]) % 5),
            int(points_dict["7"][1]),
            20,
        ):
            for b in range(
                int(points_dict["3"][0] - int(points_dict["3"][1]) % 5),
                int(points_dict["7"][0]),
                20,
            ):
                _d_point = geometry.Point(m_to_mm((b, i)))
        ...
        _a2 = radians(
            f_angle(
                f_slope(*points_dict["113"][:2], *points_dict["112"][:2]),
                f_slope(*points_dict["ПЗ42"][:2], *points_dict["113"][:2]),
            )
            - (55 + 6 / 60)
        )
        ...
        _tree = endpoint_by_distance_and_angle(
            points_dict["113"][:2],
            _l1,
            degrees(_a2)
            + np.rad2deg(
                get_angle_between_points(
                    *points_dict["113"][:2], *points_dict["112"][:2]
                )
            ),
        )
        ...
        _top_left_frame_corner = m_to_mm(
            (points_dict["3"][0] - _padding, points_dict["7"][1] + _padding)
        )
        _bottom_right_frame_corner = m_to_mm(
            (points_dict["15"][0] + _padding, points_dict["19"][1] - _padding)
        )

        _wh_dif = (
            -1 * (_top_left_frame_corner[1] - _bottom_right_frame_corner[1]),
        ...
        _main_points = [
            tuple(int(i // 100) / 10 for i in points_dict[point][:2])
            for point in "ПЗ41,111,112,113,ПЗ42".split(sep=",")
        ]
        ...
        for cord in _x_list:
            add_text(
                str(cord),
                int(cord * 1000000) + 2.5 * 2000,
                _bottom_right_frame_corner[1] + _padding / 7 * 1000,
                90,
                5,
            )
            add_text(
                str(cord),
                int(cord * 1000000) + 2.5 * 2000,
                _top_left_frame_corner[1] - _padding / 2.5 * 1000,
                90,
                5,
            )
        ...
        for cord in _y_list:
            add_text(
                str(cord),
                _top_left_frame_corner[0] + _padding / 7 * 1000,
                int(cord * 1000000) - 2.5 * 2000,
                0,
                5,
            )
            add_text(
                str(cord),
                _bottom_right_frame_corner[0] + _padding / 2 * 1000,
                int(cord * 1000000) - 2.5 * 2000,
                0,
                5,
            )
        ...
        add_text(
            f"План Масштаб 1:{ONE_TO_SCALE} Вариант: {_variant}",
            _top_left_frame_corner[0] + 10000,
            _bottom_right_frame_corner[1] - 40000,
            0,
            14,
        )
        ...
        if Path("../../../Watermark.png").exists():
            ...
            add_raster(
                "../../../Watermark.png",
                (points_dict["3"][0] / 2, points_dict["20"][1] / 2),
                0.4,
            )
